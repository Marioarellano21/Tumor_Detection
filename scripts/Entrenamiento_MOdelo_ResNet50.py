...
# Limitar uso de memoria de la GPU (4 GB en este ejemplo)
gpus = tf.config.list_physical_devices('GPU')
if gpus:
    try:
        tf.config.experimental.set_virtual_device_configuration(
            gpus[0],
            [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=4096)])  # Usa 4GB
    except RuntimeError as e:
        print("Error al configurar la memoria de GPU:", e)

from tensorflow.keras import layers, Model, callbacks
from tensorflow.keras.applications import ResNet50
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.metrics import AUC
from preprocesamiento_datos import obtener_generadores
train_gen, valid_gen, test_gen, label_encoder = obtener_generadores()
...
model.compile(
    ...
    metrics=['accuracy', AUC(name='auc')] # Métricas

# Callbacks para el entrenamiento
callbacks_list = [
    callbacks.EarlyStopping(
        monitor='val_auc',        # Métrica
        patience=5,               # Número de épocas a esperar sin mejora antes de detener, es basicamente un early stop
